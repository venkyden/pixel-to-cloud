                ...
                {steps.map((step, index) => (
                    <div key={index} className="relative animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}>
                        {index < steps.length - 1 && (
                            <div className={`absolute left-[22px] top-12 w-0.5 h-full -translate-x-1/2 ${step.status === 'completed' ? 'bg-gradient-to-b from-success to-success/50' : 'bg-border'
                                }`} />
                        )}

                        <div className={`flex items-start gap-4 p-4 rounded-xl transition-all duration-300 hover:scale-[1.02] ${step.status === 'in-progress'
